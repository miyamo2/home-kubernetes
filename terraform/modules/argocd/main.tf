locals {
  argocd_cm_patch_date      = templatestring("${path.module}/argocd-cm_patch.yaml.tftpl", { tennants = var.tenants })
  argocd_rbac_cm_patch_date = templatestring("${path.module}/argocd-rbac-cm_patch.yaml.tftpl", { tennants = var.tenants })
}
